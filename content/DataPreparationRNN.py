...
def create_datasetMultipleSeries2(dataset, n_steps_out=1, n_steps_in = 1, overlap = 1, n_features=1):
	dataX, dataY = [], []
	tem = n_steps_in + n_steps_out - overlap
	for group in range(0,n_features):
		for i in range(int((len(dataset) - tem)/overlap)):
			startx = i*overlap
			endx = startx + n_steps_in
			starty = endx
			endx=starty
			endy = endx + n_steps_out
			a= (dataset[:,group][startx:endx])
			b= (dataset[:,group][starty:endy])
			dataX.append([a,i,group])
			dataY.append([b,i,group])
	return np.array(dataX), np.array(dataY)
